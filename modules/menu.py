...
def get_full_menu_pool(client):
    ...
    """
    try:
        # utils.py dosyasında tanımladığımız isimle (Mutfak_Menu_Planlama) açıyoruz
        sh = client.open(FILE_MENU)
        ws = sh.worksheet(MENU_POOL_SHEET_NAME)
        data = ws.get_all_values()
        
        if not data: 
            st.error("Dosya bulundu ama içi boş!")
            return []
        ...
        # Hata olursa ekrana robotun mailini basalım ki kontrol edebil
        st.error(f"❌ MENÜ DOSYASINA ERİŞİLEMEDİ!")
        try:
            robot_mail = client.auth.service_account_email
            st.info(f"Kullanılan Robot Maili: {robot_mail}")
        except: pass
        
        st.warning(f"Aranan Dosya Adı: {FILE_MENU}")
        st.write(f"Teknik Hata: {e}")
        return []
...
def select_dish(pool, category, usage_history, current_day_obj, constraints=None):
    ...
    # 2. BALIK FİLTRESİ (Force edilmediyse balık gelmesin)
    ...
    for dish in candidates:
        ...
        if constraints.get('force_protein_types'):
            if dish.get('PROTEIN_TURU') not in constraints['force_protein_types']:
                continue
...
def generate_smart_menu(month, year, pool, holidays):
    ...
    # Balık Günü Belirle (Hafta içi rastgele bir gün)
