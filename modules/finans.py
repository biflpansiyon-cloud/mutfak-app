...
def distribute_yatili_installments(total_fee, year):
    try:
        ...
        if all_values and len(all_values[0]) > 0:
             first_cell = str(all_values[0][0]).lower()
...
def render_page(selected_model):
    ...
    with tab2:
        ...
        if not df_g.empty:
            if 'Ay' in df_g.columns:
                aylar = sorted(df_g['Ay'].astype(str).unique(), reverse=True)
                if len(aylar) > 0:
                    secilen = st.selectbox("Dönem:", aylar) 
                    if secilen: df_g = df_g[df_g['Ay'] == secilen]
            st.dataframe(df_g, use_container_width=True)
        else: st.warning("Veri yok.")

    # --- TAB 3: DEKONT İŞLEME (DÜZELTME ÖZELLİĞİ EKLENDİ 🛠️) ---
    with tab3:
        ...
        if files:
            ...
            if st.button("🚀 Analiz Et"):
                with st.spinner("Analiz ediliyor..."):
                    ...
                    if res:
                        ...
                        st.success("Analiz Bitti! Aşağıdan Kontrol Et 👇")
                    else: st.error("Analiz başarısız.")

            # --- DÜZELTME BÖLÜMÜ BURASI ---
            if st.session_state.get('last_analysis') and st.session_state.get('last_file_id') == sel_id:
                ...
                with st.form("dekont_duzeltme_formu"):
                    c1, c2 = st.columns(2)
                    y_ad = c1.text_input("Öğrenci Adı Soyadı", value=analiz.get('ogrenci_ad', ''))
                    y_tc = c2.text_input("TC No (Varsa)", value=analiz.get('ogrenci_tc', ''))
                    
                    c3, c4 = st.columns(2)
                    y_tut = c3.number_input("Tutar (TL)", value=float(analiz.get('tutar', 0)))
                    
                    # Tür seçimi (Dropdown)
                    tur_listesi = ["YEMEK", "TAKSİT"]
                    varsayilan_index = 0
                    if analiz.get('tur_tahmini') == 'TAKSİT': varsayilan_index = 1
                    y_tur = c4.selectbox("Ödeme Türü", tur_listesi, index=varsayilan_index)
                    
                    kaydet_butonu = st.form_submit_button("✅ Onayla ve İşle")
                    
                    if kaydet_butonu:
                        # Formdaki verilerle analiz sonucunu güncelle
                        analiz['ogrenci_ad'] = y_ad
                        analiz['ogrenci_tc'] = y_tc
                        analiz['tutar'] = y_tut
                        analiz['tur_tahmini'] = y_tur
                        ...
                        msg = ""
                        hedef_klasor = None
                        yeni_isim = sel_meta['name'] 

                        # İşleme Başla
                        ext = os.path.splitext(sel_meta['name'])[1]
                        temiz_ad = sanitize_filename(y_ad) if y_ad else "Bilinmiyor"
    ...
    with tab4:
        ...
        aylar = ["2025-Ekim", "2025-Kasım", "2025-Aralık", "2026-Ocak", "2026-Şubat"]
