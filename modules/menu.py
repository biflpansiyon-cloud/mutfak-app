...
def select_dish(pool, category, usage_history, current_day_obj, constraints=None, global_history=None):
    ...
    for dish in candidates:
        ...
        alt_tur = dish.get('ALT_TUR', '').strip() # BAKLIYAT, HAMUR vs.
        renk = dish.get('RENK', '').strip()       # KIRMIZI, BEYAZ vs.
        
        # Otomatik Yoğurt Etiketi
        ...
        # ================== YENİ GURME KURALLARI ==================
        
        # 6. BAKLİYAT ARDIŞIKLIĞI (Global History'den kontrol)
        if alt_tur == 'BAKLIYAT' and global_history:
            last_legume = global_history.get('last_legume_day', -99)
            if (current_day_obj.day - last_legume) < 3: continue # En az 3 gün ara
            
        # 7. KARBONHİDRAT POLİSİ
        # Yasaklı alt türler (Örn: Ana yemek patates ise -> Pilav/Makarna/Hamur yasak)
        if constraints.get('block_alt_types') and alt_tur in constraints['block_alt_types']: continue
        
        # 8. RENK DENGESİ (Salça kontrolü)
        if constraints.get('current_meal_colors') and renk == 'KIRMIZI':
            # Eğer menüde zaten 2 tane kırmızı varsa, 3.yü koyma
...
def get_dish_meta(dish):
    """Yemeğin özelliklerini sözlük olarak döndürür."""
...
def generate_smart_menu(month, year, pool, holidays, ready_snack_days_indices):
    ...
    # Global Hafıza (Bakliyat vb. takibi için)
    ...
    for day in range(1, num_days + 1):
        ...
        # --- ORTAK KISITLAMA OLUŞTURUCU (Helper) ---
        def build_constraints(base_cons, dish_list_for_colors=[], dish_list_for_carbs=[]):
            ...
            for d in dish_list_for_carbs:
                ...
                if alt in ['HAMUR', 'PATATES']: 
                    blocked_alts.extend(['HAMUR', 'PIRINC', 'BULGUR', 'PATATES']) # Ağır karbonhidratları yasakla
        ...
        if is_weekend:
            ...
            # Yan Ürünler
            side_cons = build_constraints({"exclude_names": daily_exclude}, [ana], [ana]) # Ana yemeğin rengi ve karbonhidratına bak
            ...
            # ORTAK YAN & ÇORBA
            shared_cons = build_constraints({"exclude_names": daily_exclude}, [ogle_ana, aksam_ana], [ogle_ana, aksam_ana]) # İki ana yemeği de analiz et
